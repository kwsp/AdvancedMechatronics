...
int main() {
    ...
    while (1) {
        ...
        if (PORTBbits.RB4 == 0) {
            // When User (B4) button is pressed, 
            // Turn on A4 for 0.5s, off for 0.5s, on for 0.5s, off for 0.5s
            _CP0_SET_COUNT(0); // CP0 counts at half the clock rate
            LATAbits.LATA4 = 1;
            while (_CP0_GET_COUNT() < 12000000) {} // 0.5s delay
            _CP0_SET_COUNT(0);
            LATAbits.LATA4 = 0;
            while (_CP0_GET_COUNT() < 12000000) {} // 0.5s delay
            _CP0_SET_COUNT(0); // CP0 counts at half the clock rate
